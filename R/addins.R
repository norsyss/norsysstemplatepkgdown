...
# TASK_NAME ----
norsyss::add_task(
  norsyss::task_inline_v1(
    name = "TASK_NAME",
    action_fn = function(data, argset, schema){
      norsyss::copy_into_new_table_where(
        table_from = "TABLE",
        table_to = "web_TABLE",
        condition = "1=1"
      )
...
addin_add_task_from_config_v8_basic <- function() {
  rstudioapi::insertText(
    '
# TASK_NAME ----
# norsyss::tm_run_task("TASK_NAME", run_as_rstudio_job_loading_from_devtools = TRUE)
norsyss::add_task_from_config_v8(
  ...
  schema = list(
    # input
    "SCHEMA_NAME_1" = norsyss::config$schemas$SCHEMA_NAME_1,

    # output
    "SCHEMA_NAME_2" = norsyss::config$schemas$SCHEMA_NAME_2
  ),
  info = "This task does..."
)
'
...
norsyss::add_schema_v8(
  name_access = c("anon"),
  name_grouping = NULL,
  name_variant = NULL,
  db_configs = norsyss::config$db_configs,
  field_types =  c(
  ...
  censors = list(
    anon = list(
      XXXX_n = norsyss::censor_function_factory_values_0_4(column_name_to_be_censored = "XXXX_n", column_name_value = "XXXX_n"),
      XXXX_pr = norsyss::censor_function_factory_values_0_4(column_name_to_be_censored = "XXXX_pr", column_name_value = "XXXX_n")
    )
  ),
  indexes = list(
    "ind1" = c("granularity_time", "granularity_geo", "country_iso3", "location_code", "border", "age", "sex", "date", "isoyear", "isoweek", "isoyearweek")
  ),
  validator_field_types = norsyss::validator_field_types_sykdomspulsen,
  validator_field_contents = norsyss::validator_field_contents_sykdomspulsen,
...
norsyss::add_schema_v8(
  ...
  censors = list(
    restr = list(
      XXXX_n = norsyss::censor_function_factory_nothing(column_name_to_be_censored = "XXXX_n"),
      XXXX_pr = norsyss::censor_function_factory_nothing(column_name_to_be_censored = "XXXX_pr")
    ),
    anon = list(
      XXXX_n = norsyss::censor_function_factory_values_0_4(column_name_to_be_censored = "XXXX_n", column_name_value = "XXXX_n"),
      XXXX_pr = norsyss::censor_function_factory_values_0_4(column_name_to_be_censored = "XXXX_pr", column_name_value = "XXXX_n")
...
TASK_NAME_action <- function(data, argset, schema) {
  ...
  if(plnr::is_run_directly()){
    # norsyss::tm_get_plans_argsets_as_dt("TASK_NAME")

    index_plan <- 1
    index_analysis <- 1

    data <- norsyss::tm_get_data("TASK_NAME", index_plan = index_plan)
    argset <- norsyss::tm_get_argset("TASK_NAME", index_plan = index_plan, index_analysis = index_analysis)
    schema <- norsyss::tm_get_schema("TASK_NAME")
  }
  ...
  # norsyss::fill_in_missing_v8(d, border = config$border)
...
TASK_NAME_data_selector = function(argset, schema){
  if(plnr::is_run_directly()){
    ...
    argset <- norsyss::tm_get_argset("TASK_NAME", index_plan = index_plan)
    schema <- norsyss::tm_get_schema("TASK_NAME")
  }

  # The database schemas can be accessed here
  d <- schema$SCHEMA_NAME$tbl() %>%
    norsyss::mandatory_db_filter(
...
#\' inside norsyss::task_from_config_v3
#\' @export
TASK_NAME_plan_analysis <- function(argset, schema) {
  if(plnr::is_run_directly()){
    argset <- norsyss::tm_get_argset("TASK_NAME")
    schema <- norsyss::tm_get_schema("TASK_NAME")
  }
